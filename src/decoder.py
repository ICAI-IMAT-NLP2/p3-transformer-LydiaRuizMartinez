...
class TransformerDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        self_attn_out = self.self_attention(
            hidden_state, hidden_state, hidden_state, mask=tgt_mask
        )
        x = x + self_attn_out
        ...
        cross_attn_out = self.cross_attention(
            hidden_state, enc_output, enc_output, mask=None
        )
        x = x + cross_attn_out
...
class TransformerDecoder(nn.Module):
    ...
    def forward(
        ...
        causal = torch.tril(
            torch.ones(seq_len, seq_len, device=x.device, dtype=torch.uint8)
        )
        tgt_mask = None
